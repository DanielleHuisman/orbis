...
export const MutationAuthToken = extendType({
    type: 'Mutation',
    definition(t) {
        // Get options
        ...
        t.boolean('requestVerifyEmail', {
            ...
            resolve(_, args: RequestArgs) {
                return orbis.transaction(async () => {
                    ...
                    const token = await generateToken(provider, TokenType.VERIFY_EMAIL, 3 * 24);
        ...
        t.boolean('requestResetPassword', {
            ...
            resolve(_, args: RequestArgs) {
                return orbis.transaction(async () => {
                    ...
                    const token = await generateToken(provider, TokenType.RESET_PASSWORD, 24);
...
export const generateToken = async (provider: Provider, type: TokenType, hoursValid: number = 24): Promise<Token> => {
